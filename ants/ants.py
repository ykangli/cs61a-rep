...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        # 需要注意move_step初始值的设定，以及min_range <= move_step <= max_range的判断条件，注意边界值
        move_step = 0
        temp_place = self.place
        min_range = self.min_range
        max_range = self.max_range
        while temp_place is not beehive:
            if temp_place.bees and min_range <= move_step <= max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        die_damage = amount + self.damage
        bees = self.place.bees[:]
        if self.armor - amount > 0:
            for every_bee in bees:
                every_bee.reduce_armor(amount)
        else:
            for every_bee in bees:
                every_bee.reduce_armor(die_damage)
        Ant.reduce_armor(self, amount)
        '''
        参考下例，减少重复代码
        bonus = True if self.armor - amount <= 0 else False
        bees = self.place.bees[:]
        for bee in bees:
            bee.reduce_armor(amount + self.damage if bonus else amount)
        Ant.reduce_armor(self, amount)
        '''
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        # 为什么？？？？？ 重写了父类的__init__ 时，要继承父类的构造方法，可以使用 super 关键字
        # 或父类名称.__init__(self,参数1，参数2，...)
        Ant.__init__(self, 1)
        self.digesting = 0
        # END Problem 6
