class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            return 'Inventory empty. Restocking required.'.format()
        else:
...
def store_digits(n):
    ...
    if (n // 10) < 1:
        ...
        num = n % 10
        n = n // 10
        example_link = Link(num)
        while n != 0:
            num = n % 10
            n = n // 10
            example_link = Link(num, example_link)
    return example_link
...
def is_bst(t):
    ...
    # 找出二分查找树t的最小节点，因为bst的性质，找最小只需要在左子树去找
    def bst_min(t):
        if t.is_leaf():
            return t.label
        return min(t.label, bst_min(t.branches[0]))

    # 找出二分查找树t的最大节点，因为bst的性质，找最小只需要在右子树去找
    def bst_max(t):
        if t.is_leaf():
            return t.label
...
def preorder(t):
    ...
    for child in t.branches:
        flatten_child += preorder(child)
    return [t.label] + flatten_child
