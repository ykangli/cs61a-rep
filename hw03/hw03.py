...
def balanced(m):
    ...
    if is_planet(m):
        ...
        left_end = end(left(m)) # 左臂挂的东西 
        right_end = end(right(m)) 
        torque_left = length(left(m)) * total_weight(left_end) # 左力矩
...
def totals_tree(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    # 如果是planet，以planet的size为根节点
    if is_planet(m):
        return tree(size(m))
    # 是mobile，求出mobile的左右分支
    else:
...
def has_path(t, word):
    ...
    if label(t) != word[0]:
        return False
    elif len(word) == 1: # 该条件暗含label(t) == word[0]
        return True
